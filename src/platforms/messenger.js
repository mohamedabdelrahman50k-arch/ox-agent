...
// GET /webhook/messenger — Meta webhook verification handshake.
export function verifyMessenger(req, res) {
  const mode = req.query['hub.mode'];
  const token = req.query['hub.verify_token'];
  const challenge = req.query['hub.challenge'];

  if (mode === 'subscribe' && token === process.env.META_VERIFY_TOKEN) {
    log('Messenger', 'ok', 'تم التحقق من الـ webhook');
...
export async function handleMessenger(req, res) {
  res.sendStatus(200);
  ...
  try {
    ...
    log('Messenger', 'ok', `رسالة من ${senderName}: "${messageText}"`);

    const history = await getConversationHistory(senderId);
    const reply = await getAIReply('messenger', senderName, messageText, history);
    ...
    logError('Messenger', 'فشل معالجة الرسالة', err);
  }
}
...
async function sendMessengerReply(senderId, reply) {
  try {
    await axios.post(
      `${GRAPH}/me/messages`,
      {
    ...
    log('Messenger', 'ok', 'تم إرسال الرد');
  } catch (err) {
    logError('Messenger', 'فشل إرسال الرد', err);
  }
}
